<?php
        ...
        {
            session_start();
            $email77=$_SESSION['fEmail2'];
        }
        else
        {
            $email77=$_POST['email77'];
            session_start();
            $_SESSION['fEmail2']=$email77;
        }
        ...
        $sql="select email from user_details where email='$email77'";
        $result=mysqli_query($con,$sql);
        if($result->num_rows==0)
        {
            $sql2="select email from blocked_users where email='$email77'";
            $res2= mysqli_query($con,$sql2);
            if($res2->num_rows==0)
            {
                echo "<script type='text/javascript'>window.alert('Email not registered.');</script>";
                echo "<meta http-equiv='refresh' content='0;url=index.php?flag13=1'>";
            }
            else
            {
                echo "<script type='text/javascript'>window.alert('Email is Blocked.');</script>";
                echo "<meta http-equiv='refresh' content='0;url=index.php?flag13=1'>";
            }
        }
        else
